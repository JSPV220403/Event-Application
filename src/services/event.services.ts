      ...
      .map((event) => ({
        ...
        image_url:
          event?.image_url != null
            ? `http://localhost:8000/eventImages/${event?.image_url}`
            : null,
      ...
      result.map(async (event) => {
        const schedulesWithSeats = await Promise.all(
          event.schedule.map(async (schedule) => {
            ...
            const bookedSeats = Number(filledSeats._sum.seat_count ?? 0);
            ...
            return {
              ...
              availableSeats,
              bookedSeats:
                bookedSeats == schedule?.venue_capacity ? 0 : bookedSeats,
            };
          }),
        );
